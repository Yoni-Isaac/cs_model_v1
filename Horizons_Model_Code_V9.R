...
if(Type_of_runing=="u_t"){
  ...
  nodes_links_df=nodes_linker(current_line,lines_db,horizons_db,cs_id)
}
# Func
nodes_linker=function(current_line,lines_db,horizons_db,cs_id){
  ...
  for (i in 1:nrow(lines_db)){
    ...
    # Get Horizon data
    horizons_db_st_i=dplyr::filter(horizons_db_st,cs_id == cs_id)
