...
library(leaflet)
# Unit = Top Senon
MS_work_zone=st_read(paste0(Background_path,"/Apps/External_Data/Geology_Model_Moac_Elements/5-Senon_Update_polys.shp"))  %>% st_transform(.,4326) # %>% st_union(.)
outcrop_MS =raster(paste0(Background_path,"/Apps/External_Data/Geology_Model_Moac_Elements/MOUNT_SCOPUS_outcrops.tif")) %>% 
  projectRaster(.,crs=4326)
pperhorizon_MS=raster(paste0(Background_path,"/Apps/External_Data/Geology_Model_Moac_Elements/AVEDAT_cf_4.tif")) %>% 
  projectRaster(.,crs=4326)
#inside paramters
Active_F=sf::st_read(paste0(Background_path,"/Apps/External_Data/Geology_Model_Moac_Elements/Active_F_EastMt.shp"))%>%
  st_transform(.,crs = "+proj=longlat +datum=WGS84") %>%
  subset(.,,c("geometry")) %>% st_zm(.)
dtm=raster(paste0(Background_path,"/Apps/CS_Model_V02/data/DEMs/DTM.tif"))
folderPath=paste0(Background_path,"/Apps/External_Data/Geology_Model_Moac_Elements/CS_csv")
ob_wells=read_excel(paste0(Background_path,"/Apps/External_Data/Geology_Model_Moac_Elements/ob_wells_EastMt_V1.xlsx"),sheet="ob_wells") 
geology_50=sf::st_read(paste0(basemap_pth,"geology_50_V6.shp"))
geology_200=sf::st_read(paste0(basemap_pth,"geology_200_V4.shp"))
outcrops2horizons_units=read.csv(paste0(Background_path,"/Apps/External_Data/Geology_Model_Moac_Elements/outcrops2horizons_units.csv"))
source("G:/Temporary_Scripts/idw_try.R")
# Func ####################
tictoc::tic()
horizon_prod_lst=line2horizon(work_zone=work_zone,                               ## Main boundry of the model with inner boundris
                              unit_bound=unit_bound,                             ## boundary of the interpolation
                              Horizon_v="JUDEA LOWER",                                  
                              group_v=7,
                              useoutcrop="NO", # "points" # "NO"                 ## outcrops for the interpolation
                              outcrop_r=outcrop_JL)  # outcrop_avedat NULL      ## outcrops for upper cutting
horizon_fix_lst=fixbyupper(horizon=horizon_prod_lst$horizon_rs_cf,
                           upperhorizon=upperhorizon_JL)
tictoc::toc()
