...
if(Type_of_runing=="u_t"){
  ...
  horizons_db=read.csv(paste0(Background_path,'/Apps/External_Data/horizons_db.csv'))
  nodes_links_df=nodes_linker(current_line,lines_db,horizons_db,charts)
}
# Func
nodes_linker=function(current_line,lines_db,horizons_db,charts){
  horizons_db_st=st_as_sf(horizons_db, coords = c("Longitude", "Latitude"), crs = 4326,remove=F) %>% 
    mutate(cs_id=str_sub(ID,1,1))
  current_pnts=st_as_sf(charts$cs_data$CS_dtm_df,
                       coords = c("Longitude", "Latitude"), crs = 4326,remove=T)
  ...
  for (i in 1:nrow(lines_db)){
    ...
    if(nrow(juc_pnt_i)){
      ...
      nn_i_horizons=horizons_db_st_i %>% 
        ...
        mutate(closest_pnt=unlist(try({map(data,
                                           function(df) nngeo::st_nn(juc_pnt_i,df$geometry))},silent = T))) 
      
      if(nrow(nn_i_horizons)>0){
        ...
        # Get Distance data (Extract from Current CS) 
        juc_i_horizons$Distance = current_pnts[as.numeric(unlist(nngeo::st_nn(juc_pnt_i,current_pnts))),]$Distance
