...
cs_model=function(in_param)
{
  ...
  if(class(in_param)=="character"){
    ...
    drawn_polyline=st_read(paste0(Background_path,'/Apps/External_Data/drawn_polyline.csv')) %>% st_set_crs(4326)
  ...
  dem_pth = "data/DEMs"
  drawn_polygon=st_buffer(drawn_polyline,dist=1000)
  ...
  if(exists("DEM_rst",where=additional_layers_lst)==T) {
    ...
    # Try Get Additional
    crop_status="try-success"
    DEM_rst_i=try(raster::crop(additional_layers_lst$DEM_rst,drawn_polygon))
    crop_status=class(DEM_rst_i)
    if(crop_status!="try-error"){
      DEMs.list[i+1]=DEM_rst_i
      static_DEM_i=i+1
    } else {
      messeges_str=as.character("Automatic coordinate network conversion failed. The run may fall, please perform a conversion to the WGS84 network manually.")
  ...
  # 3.3.1 Set DEMs Colors ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  message("3.3.1 Set DEMs Colors")
  ...
  # 3.3.2 Set Geo Map Colors ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  message("3.3.2 Set Geo Map Colors")
