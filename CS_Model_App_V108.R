...
cs_id_i=1
load_sub=ifelse(tor=="w",source,source)
...
load_sub('scripts/CS_Model_Code_V48.R', encoding = 'UTF-8') #debugSource
load_sub('scripts/CS_Chart_Code_V3.R', encoding = 'UTF-8') #debugSource
...
### TJ Faluts ------------------------------------------------------------------
transforms_ll=sf::st_read(transforms_pth,options = "ENCODING=UTF-8")
...
library_dms=sf::st_read(paste0(dem_pth,"/library_dms_V2",".shp"),options = "ENCODING=UTF-8")%>% 
  dplyr::filter(.,basin != "national")

### Geology --------------------------------------------------------------------
geology_50=sf::st_read(paste0(basemap_pth,"geology_50_V6.shp"),options = "ENCODING=UTF-8")
geology_50_proxy=geology_50[1,]
geoIDX_50=read_excel(paste0(design_pth,"/INDEX_National_V6.xlsm"),sheet = "geoINDEX50")

geology_200=sf::st_read(paste0(basemap_pth,"geology_200_V4.shp"),options = "ENCODING=UTF-8")
geology_200_proxy=geology_200[1,]
geoIDX_200=read_excel(paste0(design_pth,"/INDEX_National_V6.xlsm"),sheet = "geoINDEX200")
...
# Geology - Tiles Grid
geogrid= dplyr::filter(sf::st_read(paste0(basemap_pth,"geogrid_V10.shp"),options = "ENCODING=UTF-8"),!is.na(resoid) & resoid!=0,)
...
### Indexes --------------------------------------------------------------------
INDEX <<- as.data.frame(read_excel(paste0(design_pth,"/INDEX_National_V6.xlsm"),sheet = "Index"))
INDEX_DEMs = as.data.frame(read_excel(paste0(design_pth,"/INDEX_National_V6.xlsm"),
                                      sheet = "Index_DEMs",
...
geomdl_palette=c("#aeafb0","#a2cffc","#95b9de","#467bb3","#1adb9b","#1EDC66","#1EDC21","#0FA411",
      ...
      navbarPage(id = "tabs",
                 ...
                 tabPanel("Home", icon = icon("home"),
                          ...
                          HTML("<div style='vertical-align:middle; text-align:center'> 
                                  <img src='app_panel_251.jpg' width='1400' height='500' alt='This is alternate text'></img>
                             </div>"),
                          HTML("<div style='vertical-align:middle; text-align:center'> 
                 ...
                 tabPanel("Map", icon = icon("location-arrow"),
                          ...
                          fluidRow(
                            ...
                            column(width = 5, style = "background-color:#2c3e50; opacity: 0.8;",
                                   ...
                                   column(width = 3,offset = 0.5,
                                          fileInput('file1',
                                                    creditactionBttn(infoId="Refresh_info",color="default",c_label="Upload wells",
                                                                     icon= tags$i(class = "fas fa-file-upload", style="font-size: 34px; color: black")),
                                                    accept = c(
                                                      'text/csv',
                                   ...
                                   column(width = 3,offset = 0.5,
                                          fileInput('csgrid',
                                                    creditactionBttn(infoId="csgrid_info",color="default",c_label="Upload CS Grid",
                                                                     icon= tags$i(class = "fas fa-file-upload", style="font-size: 34px; color: blue")),
                                                    accept = c(
                                                      'text/csv',
                                                      'text/comma-separated-values',
                                                      'text/tab-separated-values',
                                   ...
                                   column(width = 3,offset = 0.5,
                                          fileInput('cspic',
                                                    creditactionBttn(infoId="cspic_info",color="default",c_label="Upload CS pictures",
                                                                     icon= tags$i(class = "fas fa-file-image", style="font-size: 34px; color: blue")),
                                                    accept = c(
                                                      '.csv',
                                                      '.png',
                                                      ".jpg"
                                          ...
                                          selectInput(inputId="picslct",
                                                      label = "Select CS Picture",
                                                      choices=c(cs_choices),
                                                      selectize=TRUE,
                                                      selected =cs_choices[1],
                                                      multiple=F),
                                          # Fit CS picture  Parameters ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                                          uiOutput("picUImap")       
                                   )
                            )
                          ),
                 ...
                 tabPanel("Cross Section panel",  icon = icon("google-wallet"),
                          ...
                          fluidPage(
                            # CSS Elements
                            tags$style(HTML("
                                  /* Custom CSS styles for the absolutePanel */
                                  #initiation {
                                ...
                                ")),
                            absolutePanel(id = "initiation",
                                          style="z-index:500;",
                                          class = "panel panel-default",
                                          fixed = TRUE,
                                          draggable = TRUE, top = 170, left = 180, right = "auto", bottom = "auto",
                                          ...
                                          checkboxInput("mode",
                                                        msgactionBttn(infoId="mode_info",
                                                                      color="default",
                                                                      c_label="Full CS"),
                                                        value = T)
                            ...
                            column(2,
                                   ...
                                   titlePanel("Cross Section Parameters"),
                                   ...
                                   numericInput("h_resolution",  msgactionBttn(infoId="h_resolution_info",color="default",c_label="Horizontal Grid Resolution [m]:"),
                                                min = 10, max = 1000, value = 200,step=10),
                                   # Vertical Resolution ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                                   numericInput("v_resolution",  msgactionBttn(infoId="v_resolution_info",color="default",c_label="Vertical Grid Resolution [m]:"),
                                                min = 1, max = 500, value = 100,step=5),
                                   # Title ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                                   ...
                                   # Use CS picture - Activation ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                                   checkboxInput("pic_activation",
                                                 msgactionBttn(infoId="pic_activation_info",color="primary",c_label="Use CS picture as backround:")),
                                   # Fit CS picture  Parameters ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                                   uiOutput("picUI")
                            ...
                            column(9,offset=0,
                                   ...
                                   fluidRow(downloadButton('cs_download_ppt', 'Download Editable'),
                                            ...
                                            downloadButton('cs_download_excel', 'Download CS data')
                                   )
                 ...
                 tabPanel(title="Build Geology Model",icon = icon("accusoft"),
                          fluidRow(
                            ...
                            column(width = 2,
                                   ...
                                   fluidRow(
                                     column(width = 7,
                                            selectInput("observation_points", msgactionBttn(infoId="observation_points_info",color="primary",c_label="Observation Points:"),
                                                        multiple=T,
                                                        choices=NULL),
                                     ),
                                     column(width = 7,
...
server <- function(input, output, session) {
  ...
  observeEvent(input$csgrid_info,once = F, handlerExpr={msgalart("Upload Grid of Cross Sections","<p style='text-align:justify;'>Based on the horizon lines that you create at the past. For more details, see the software guide - Part B</a>. </p>")})
  observeEvent(input$cspic_info,once = F, handlerExpr={msgalart("Load metadata file of cross section's images","<p style='text-align:justify;'> The file structure includes the local link to the files and the line structure in the Excel file. For more details, see the software guide - Part A</a>. </p>")})
  ...
  observeEvent(input$h_resolution_info,once = F, handlerExpr={msgalart("Horizontal Resolution","By this paramter you can to set the resoloution in Y axis.")})
  observeEvent(input$v_resolution_info,once = F, handlerExpr={msgalart("Vertical Resolution","By this paramter you can to set the resoloution in X axis.")})
  ...
  observeEvent((input$login | !is.null(input$file1)), {
    ...
    horizons_db<<-NULL
    {
      if(!is.null(input$file1)){
        ...
        # Sand DB's to Environment ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        Geology_Description_ss<<-Geology_Description_ss
        CS_model_system_sf<<-CS_model_system_sf
  ...
  observeEvent(input$additional_layers,{
    ...
    if(inFile$type=="image/tiff" & input$additional_layers_type=="Digital Terrain Model (raster)"){
      ...
      if(as.character(crs(DTM_rst))!="+proj=longlat +datum=WGS84 +no_defs"){
        ...
        # Set Extent 
        ext_sp <- as(extent(DTM_rst), 'SpatialPolygons')
        ext_pnt <- st_as_sf(ext_sp) %>% st_cast("POINT")
      ...
      # Show user DTM bounds on map
      ext_pnt_line = st_as_sf(st_union(st_as_sf(as_sfnetwork(ext_pnt_ll), "edges")))
      proxy_mainmap=add_element(main_map=proxy_basemap,
                                ad_lyr=ext_pnt_line,
                                type="Digital Terrain Model (raster)")
    ...
    # Geological layer (raster)
      ...
      if(as.character(crs(DEM_rst))!="+proj=longlat +datum=WGS84 +no_defs"){
        ...
        # Show user DEM bounds on map
        ext_pnt_line = st_as_sf(st_union(st_as_sf(as_sfnetwork(ext_pnt_ll), "edges")))
        proxy_mainmap=add_element(main_map=proxy_basemap,
                                  ad_lyr=ext_pnt_line,
                                  type="Geological layer (raster)")
    ...
    if(tor=="t"){
      # %%%%%%%%%%%%%%%%%%%%%%%%% Test Elements %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      saveRDS(additional_layers_lst,file="G:/Apps/External_Data/QA_elements/additional_geology.RData") 
      # %%%%%%%%%%%%%%%%%%%%%%%%% Test Elements %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    }
  })
  
  ### Load CS's Pictures -------------------------------------------------------
  # Set reactive CS' files ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  picture_rct <<- reactive({ 
    req(input$cspic) 
    picFile=input$cspic %>% 
      separate(name, into = c("name", "type"), sep = "\\.")
    
    # Read metadata file
    pics_df = dplyr::filter(picFile,type %notin% "csv")
    pic_md = left_join(read_csv(dplyr::filter(picFile,type %in% "csv")$datapath),pics_df) 
    
    
    # Check meta data file
    dfcolnames = c("name","distance","X","Y")
    if(all(dfcolnames %in% colnames(pic_md))==F){
      showModal(modalDialog(
        title = "There seems to be some issues with the data you entered: ",
        "Check that you have entered a meta data file that includes the following 5 fields: name|distance|X|Y",
        easyClose = TRUE,
        footer = NULL
      ))
    }
    # Check Pictures
    cs_na = nrow(dplyr::filter(pic_md, is.na(datapath)))
    if(cs_na>0){
      showModal(modalDialog(
        title = "There seems to be some issues with the data you entered: ",
        "Check that sll your CS picture includ meta-data details.",
        easyClose = TRUE,
        footer = NULL
      ))
    }
    ...
    # Check the CRS
    if(max(pic_md$X)<100){
      pics_shp <- st_as_sf(pic_md,coords = c("X", "Y"),crs=4326)
    } else {
      pics_shp <- st_as_sf(pic_md,coords = c("X", "Y"),crs=crs_id$crs) %>%  st_transform(.,crs=4326)
    }
    
    
    
    # Update reactive CS' picture list ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    updateSelectInput(session,
                      inputId="picslct",
                      selected=pics_shp$name[1],
                      choices = pics_shp$name
    )
    return(pics_shp)
  })
  
  
  observeEvent(list(input$cspic,input$picslct),{
    ### Base Map - Change proxy map --------------------------------------------
    req(picture_rct())
    req(input$picslct != "Inactive")
    
    # Build Full LINESTRING
    all_lines = picture_rct() %>% 
      dplyr::arrange(name,distance) %>% 
      group_by(name,datapath)  %>%
      dplyr::summarise(do_union=F) %>%
      sf::st_cast("LINESTRING")
    
    pics_line <<- dplyr::filter(all_lines,name %in% input$picslct)
    ...
    proxy_mainmap=proxy_basemap %>% 
      clearGroup(group="pics_lines") %>% 
      # All lines
      addPolylines(data=all_lines, 
                   color="red",
                   dashArray='5,10',
                   label = ~name,
                   labelOptions=labelOptions(textsize = "30px"),
    ...
    # Set Fitting Elements ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    pics_pnt = dplyr::filter(picture_rct(),name %in% input$picslct)
    
    
    minx = min(pics_pnt$distance,na.rm = T)
    maxx = max(pics_pnt$distance,na.rm = T)
    miny = min(pics_pnt$elvation,na.rm = T)
    maxy = max(pics_pnt$elvation,na.rm = T)
    
    output$picUI=renderUI({
      column(width = 9, style = "background-color:#6584a3; opacity: 0.9; color:#000000 ;border-radius: 15px 50px 30px;",
             # X Stretch ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
             sliderInput(inputId="picxstr",
                         label="X-axis stretch:",
                         min = minx*2, max = maxx*2, value = c(minx,maxx),step=1),
             # Y Stretch ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
             sliderInput(inputId="picystr",
                         label="Y-axis stretch:",
                         min = miny*2, max = maxy*2, value = c(miny,maxy),step=1),
             # Transparency ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
             sliderInput(inputId="pictras",
                         label="Backround transparency:",
  ...
  observeEvent(list(input$picslct,input$pic_activation),{
    req(picture_rct())
    req(!is.null(charts))  
    pics_pth = pics_line$datapath[1]
    pic <<- readPNG(pics_pth) # readPNG
    print("AAAA")
    cs_pic <<- charts$cs_raw
  })
  
  # Final corrections of the picture ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
  observeEvent(list(input$pic_activation,input$picxstr,input$picystr,input$pictras),{
    req(!is.null(charts))
    req(!is.null(pic))
    ### Add background picture to preview visualization ------------------------
    pica <- matrix(rgb(pic[,,1],pic[,,2],pic[,,3], pic[,,4] * input$pictras/100), nrow=dim(pic)[1])
    x_min <- input$picxstr[1]
    x_max <- input$picxstr[2]
    y_min <- input$picystr[1]
    y_max <- input$picystr[2]
    
    cs_pic = charts$cs_raw + 
      theme_half_open()+
      annotation_raster(pica, xmin = x_min, xmax = x_max, ymin = y_min, ymax = y_max) +
      coord_cartesian(xlim = c(x_min, x_max), ylim = c(y_min, y_max)) +
      geom_blank() +
      theme_void() +
      theme(legend.position="NUN")   
    charts$cs_pic <<- cs_pic
    ### Render CS View ---------------------------------------------------------
  ...
  data_slc=reactive({
    ...
    if(feature_type=="Line") {
      ...
      if(input$country!="Indefinite") {
        crs_id=as.numeric(subset(localtiles_df,country==input$country,crs))
      } else {
        crs_id=4326
      }
      
      # Create CS line as single line
      drawn_polyline_st <- st_as_sf(as.data.frame(polyline_coordinates_mt),
                                    coords = c("V1", "V2"), crs = crs_id,remove=T) %>% 
        as_sfnetwork() %>% 
        st_as_sf(., "edges") %>% 
        st_union() %>% 
        st_as_sf()
      ...
      drawn_polygon = st_buffer(drawn_polyline_st, dist=input$Buffer/100) 
      CS_wells_coordinates=st_as_sf(CS_model_system,coords = c("Longitude","Latitude"),crs=crs_id)
      ...
      # selected Faults
      transforms_st=st_as_sf(transforms_shp) ;st_crs(transforms_st)=crs_id
      ...
      if(tor=="t"){
        ...
        st_write(drawn_polyline_st, paste0(Background_path,'/Apps/External_Data/drawn_polyline.csv'),
               ...
               {if(!is.null(input$file1) & is.null(input$file2)){
                 {
                   ...
                   withProgress(message = 'Cross Section building in progress',
                                ...
                                expr = {
                                  ...
                                  CS_Prameters<-list(
                                    ...
                                    h_resolution=input$h_resolution,
                                    v_resolution=input$v_resolution,
                   ...
                   output$cs_download_excel <-  downloadHandler(
                     message("Download Cross Sections"),
                     filename = function() { 
                       paste("Data_CS-", Sys.Date(), ".xlsx", sep=""
                       )
                     },
                     content = function(file) {
                       openxlsx::write.xlsx(charts$cs_data, file = file)
                     })  
      ...
      input$Select_horizon_by},{
        ...
        if(is.null(cs_tagging)==T){
          output$cs_tagging<-renderPlot({
            ...
            # Set Current CS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            if(isTRUE(input$pic_activation)){
              cs4tagging = charts$cs_pic
            } else{ cs4tagging = charts$cs_raw }
  ...
  observeEvent(input$repair,{
    ...
    req(input$Select_horizon_by=="Hydrogeology libraries")
  ...
  observeEvent(input$tabs,{
    ...
    ### Observation Points (Passive update) ------------------------------------
    if(!is.null(Geology_Description_ss)==T){
      
      Geology_Description_trg<<-geo_obs_updt(INDEX,
                                             Geology_Description_ss,
                                             CS_type=input$CS_type)
        ...
        if(as.character(geo_dims_v()) %in% c("2D", "Edit")){
          ...
          bootstrapPage(
            ...
            absolutePanel(id = "controls",
                          ...
                          h2("Uncertainty Level:"),
                          plotlyOutput("geo_valid", height = 250),
                          DT::dataTableOutput("geo_obj")
            ) 
  ...
  ## Set RGM Mode ==============================================================
  output$rgmUI=renderUI({
    fluidRow(
      column(width = 7,
             actionButton("RGM",
                          icon = icon("accusoft"),
                          style="color: #fff; background-color: #6d94b5; border-color: #2e6da4",
                          width = "200px",
                          label = HTML("<span style='font-size:1.3em;'><br />Run</span>")
             ),
      )      
  ...
  ### Observation Points (Switch Mode) ------------------------------------------
  observeEvent(input$CS_type,{
    if(!is.null(Geology_Description_ss)==T){
  ...
  observeEvent(input$upper_layer,{
    # Get files
    inFile=input$upper_layer %>% dplyr::filter(type == "image/tiff")
    req(nrow(inFile)>0)
    # Build union upper layer
    for( i in 1: nrow(inFile)) {
      upper_rst_i=raster(inFile$datapath[i])
      if(as.character(crs(upper_rst_i))!="+proj=longlat +datum=WGS84 +no_defs"){
        upper_rst_i=projectRaster(upper_rst_i, crs = 4326)
      }
      if(i==1){
        upper_rst=upper_rst_i
      } else {
        upper_rst_rs <- resample(upper_rst,upper_rst_i)
        b <- brick(upper_rst_i, upper_rst_rs)
        upper_rst <- min(b, na.rm = TRUE)
      }
    } 
  ...
  observeEvent(input$surface_unit,{
    surface_unit_st <<- geology_map_act %>% dplyr::filter(Name_Eng %in% input$surface_unit)
    if(nrow(surface_unit_st)>0){
      proxy_geo2d_map=leafletProxy(
        mapId = "geo2d_map",
        session = session
      ) %>%
        clearGroup(group="geo_surf") %>% 
        addPolygons(data=surface_unit_st,
                    color= ~geopal(Code),
                    fillColor= ~geopal(Code),
                    label = ~Name_Eng,
                    labelOptions=labelOptions(textsize = "15px",
  ...
  observeEvent(input$observation_points,{
    req(!is.null(Geology_Description_ss)==T)
    ...
    obs_points_i <<- Geology_Description_ss %>% 
      ...
      subset(.,,c("well_id","name","Longitude","Latitude","elv","top_layer","bot_layer",input$CS_type)) %>% 
      left_join(.,subset(INDEX_obs,,c(input$CS_type,"f_name"))) %>% 
      dplyr::filter(f_name %in% input$observation_points) %>% 
      dplyr::distinct_all(,.keep_all = T)
    
    
    if(input$horizon_type=="Base"){
      obs_points_u <<- obs_points_i %>% group_by(well_id,name,elv,Longitude,Latitude) %>% 
        dplyr::summarise(targ_dpt=max(bot_layer,na.rm = T),
                         targ_elv=elv-targ_dpt) 
    } else{
      obs_points_u <<- obs_points_i %>% group_by(well_id,name,elv,Longitude,Latitude) %>% 
        dplyr::summarise(targ_dpt=min(top_layer,na.rm = T),
                         targ_elv=elv-targ_dpt) 
    }
    obs_pal <- colorNumeric(geomdl_palette, obs_points_u$targ_elv, na.color = "transparent")
    
    if(nrow(obs_points_u)>0){
      proxy_geo2d_map=leafletProxy(
        mapId = "geo2d_map",
        session = session
      ) %>%
        clearGroup(group="geo_obs") %>% 
        addCircleMarkers(
          label =~paste0("name=",as.character(name)," ;z=",as.character(round(targ_elv,0))),
          data=obs_points_u,
          fillOpacity = 1,
          color = ~obs_pal(targ_elv),
          stroke = FALSE,
          radius =6,
          group="geo_obs" 
        ) 
  ...
  observeEvent(input$RGM,{
    ...
    ### Update 2D map ----------------------------------------------------------
    obs_points4map <<- left_join(obs_points_u,
                                 subset(Geology_Description_ss,,c("well_id","name","Longitude","Latitude"))) %>% 
      dplyr::distinct(.,well_id,.keep_all = T)
    horizons_db4map  <<- dplyr::filter(horizons_db_i,ID %notin% input$notincluded)
    
    # Build Geomodel as html ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    geo2d_map=leaflet(options = leafletOptions(zoomControl = F)) %>% 
      setView(lng=35.2,lat=32.55,zoom=10) %>%
      addProviderTiles(providers$CartoDB.Positron) %>%
      htmlwidgets::onRender("
                function(el,x) {
    ...
    if (!is.null(geomdl$above_rst)==T){
      geo2d_map_full <<- extra4geo2d(geo2d_map=geo2d_map,
                                     horizons_db_i=horizons_db4map,
                                     geomdl,
                                     obs_points = obs_points4map,
                                     horizon_unit=input$horizon_unit)
    } else {
      geo2d_map_full <<- updt_geo2d_map(geo2d_map=geo2d_map,
                                        horizons_db_i=horizons_db4map,
                                        geomdl,
                                        obs_points = obs_points4map,
                                        horizon_unit=input$horizon_unit)
    ...
    # Standalone Mode ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    if(tor!="w"){
      output$geo2d_map <<- renderLeaflet({geo2d_map_full})
      proxy_geo2d_map=leafletProxy(
        mapId = "geo2d_map",
        session = session
      )  
      ...
      output$geo2d_map <<- renderLeaflet({geo2d_map_partial})
      proxy_geo2d_map=leafletProxy(
        mapId = "geo2d_map",
        session = session
      )  
    ...
    output$rgmUI=renderUI({
      fluidRow(
        column(width = 7,
               actionButton("RGM",
                            icon = icon("accusoft"),
                            style="color: #fff; background-color: #147ed9; border-color: #2e6da4",
                            width = "200px",
                            label = HTML("<span style='font-size:1.3em;'><br />Run</span>")
               ),
        ),
        column(width = 3,
               downloadButton('rgm_download', '')
        )
      )
    })
    ...
    intonhor=data.frame(int_elv=raster::extract(geomdl$rst,horizons_db4val),horizons_db4val[c("ID","Elevation")]) %>% 
      dplyr::filter(!is.na(int_elv))
    hor_rsq=round(rsq(intonhor$Elevation,intonhor$int_elv),2)
    hor_rmsd=round(bio3d::rmsd(intonhor$Elevation,intonhor$int_elv),2)
    objval_df=data.frame(Name=c("CS's"),RSQ=c(hor_rsq),RMSD=c(hor_rmsd))
    
    valid_C = ggplot()+
      geom_point(data=intonhor,aes(x=Elevation,y=int_elv,text =  paste("CS: ", ID)),color="red",alpha=0.5)+
      labs(x="Measure & Estimate",y="Model")
    
    ### Observation point ------------------------------------------------------
    if (!is.null(input$observation_points)==T){
      obs_points4val=st_as_sf(obs_points4map,coords=c("Longitude","Latitude"),crs=4326) 
      intonobs=data.frame(int_elv=raster::extract(geomdl$rst,obs_points4val),obs_points4val[c("name","targ_elv")]) %>% 
        dplyr::filter(!is.na(int_elv))
    ...
    output$geo_valid=renderPlotly({
      valid_CI
    })
    output$geo_obj=renderDT({
      objval_df
    })
